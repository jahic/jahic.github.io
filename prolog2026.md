## Prolog 2026
<br/>
##### 26.09.2026
##### Biznis prilike sa vještačkom inteligencijom
<br/>
##### Jasmin Jahic, University of Cambridge,UK
<br/>

#### Prezentacija:
##### - <a href="doc/tutorial/prolog2026/prolog2026.pdf">Prolog 2026</a>

#### Dodatni materijal:
##### Tabela: 
<table>
    <thead>
        <tr>
            <th>Kurs</th>
            <th>Studenti</th>
        </tr>
    </thead>

    <tbody>
        <tr>
            <td>Softverske arhitekture</td>
            <td>Amy, Hasan, Hannah, Stewart, Li</td>
        </tr>

        <tr>
            <td>Sigurnost</td>
            <td>Amy, Hasan, Hannah, Stewart, Li, Stewart</td>
        </tr>

        <tr>
            <td>Softverski inženjering</td>
            <td>Amy, Hasan, Hannah, Stewart, Li, Luka</td>
        </tr>

        <tr>
            <td>AI</td>
            <td>Emir, Damir, Emina, Luka</td>
        </tr>

        <tr>
            <td>Arhitektura računara</td>
            <td>Amy, Hasan, Hannah, Damir, Emina</td>
        </tr>

        <tr>
            <td>Kompajleri</td>
            <td>Amy, Hasan, Hannah, Damir, Emina</td>
        </tr>

        <tr>
            <td>Mašinsko učenje</td>
            <td>Amy, Hasan, Hannah, Damir, Leon</td>
        </tr>

        <tr>
            <td>Objektno-orjentisano programiranje</td>
            <td>Francois, Hannah, Steward, Li, Luka</td>
        </tr>

        <tr>
            <td>Matematika 1</td>
            <td>Francois, Hannah, Steward, Li, Luka</td>
        </tr>
    </tbody>
</table>

#### Tekst:
##### "There is no struggle too vast, no odds too overwhelming, for even should we fail - should we fall - we will know that we have lived."

#### Slike:
##### - <a href="doc/tutorial/prolog2026/Neuron.jpg">Neuron</a>
##### - <a href="doc/tutorial/prolog2026/CoatOfArms.jpg">CoatOfArms</a>
##### - <a href="doc/tutorial/prolog2026/LibraryImage.jpg">LibraryImage</a>
##### - <a href="doc/tutorial/prolog2026/Data.xlsx">Data.xlsx</a>